...
enum METFilterIndex{
	...
	idx_PassBadPFMuonFilterDz_Update
...
class METScanningNtupleMakerMINIAOD : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
	...
	private:
		...
		edm::EDGetTokenT<bool> badPFMuonFilterUpdateDzToken_;
		...
		bool PassBadPFMuonFilterDz_Update;
...
METScanningNtupleMakerMINIAOD::METScanningNtupleMakerMINIAOD(const edm::ParameterSet& iConfig)
	:
		...
		badPFMuonFilterUpdateDzToken_(consumes<bool>(iConfig.getParameter<edm::InputTag>("BadPFMuonFilterUpdateDz"))),
...
METScanningNtupleMakerMINIAOD::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
	...
	//std::cout<<"+++++++++++++++++++++++++++++++++++++++++++++++++++++++++"<< std::endl;
	//std::cout<<"runno:"<<_runNb<<":"<<_lumiBlock<<":"<<_eventNb<<std::endl;
	//	bool run_found;
	//	run_found=false;
	...
	edm::Handle<bool> handle_PassBadPFMuonFilterUpdateDz;
	iEvent.getByToken(badPFMuonFilterUpdateDzToken_,handle_PassBadPFMuonFilterUpdateDz);
	if(handle_PassBadPFMuonFilterUpdateDz.isValid())PassBadPFMuonFilterDz_Update =  (*handle_PassBadPFMuonFilterUpdateDz );
	else{  std::cout <<"handle_PassBadPFMuonFilterDz_Update.isValid =false" <<endl; PassBadPFMuonFilterDz_Update = true;}
	...
	if( !trigResults.failedToGet() ) {
		...
		for( int i_Trig = 0; i_Trig < N_Triggers; ++i_Trig ) {
			if (trigResults.product()->accept(i_Trig)) {
				...
				if(TrigPath.Contains("HLT_PFHT450_SixPFJet36_PFBTagDeepCSV_1p59_v"))HLT_PFHT450_SixPFJet36_PFBTagDeepCSV_1p59 = true;
	...
	for( pat::MuonCollection::const_iterator imuon = (*muonCollection_).begin() ; imuon != (*muonCollection_).end() ; imuon++) {
		...
		_muonCharge.push_back(imuon->charge());
		...
		*/
		...
		_muonIsoSumDR04.push_back((imuon->pfIsolationR04().sumChargedHadronPt + max(0., imuon->pfIsolationR04().sumNeutralHadronEt + imuon->pfIsolationR04().sumPhotonEt - 0.5*imuon->pfIsolationR04().sumPUPt))/imuon->pt());
	...
	for( pat::ElectronCollection::const_iterator gsfiter = (*electronCollection_).begin() ; gsfiter != (*electronCollection_).end() ; gsfiter++ ) {
		...
		_eleCharge.push_back( gsfiter->charge());

		gsf_VID_cutBasedElectronID_Fall17_94X_V2_loose.push_back( int(gsfiter->electronID("cutBasedElectronID-Fall17-94X-V2-loose")));
		gsf_VID_cutBasedElectronID_Fall17_94X_V2_medium.push_back( int(gsfiter->electronID("cutBasedElectronID-Fall17-94X-V2-medium")));
		gsf_VID_cutBasedElectronID_Fall17_94X_V2_tight.push_back( int(gsfiter->electronID("cutBasedElectronID-Fall17-94X-V2-tight")));
		gsf_VID_cutBasedElectronID_Fall17_94X_V2_veto.push_back( int(gsfiter->electronID("cutBasedElectronID-Fall17-94X-V2-veto")));
	...
	for(pat::PackedCandidateCollection::const_reverse_iterator p = pfcands->rbegin() ; p != pfcands->rend() ; p++ ) {
		...
		_PFcand_charge.push_back(p->charge());
...
*/
	...
	outputTree->Branch("PassBadPFMuonFilterDz_Update", &PassBadPFMuonFilterDz_Update, "PassBadPFMuonFilterDz_Update/O");
	...
	outputTree->Branch("_eleCharge", &_eleCharge);
	outputTree->Branch("gsf_VID_cutBasedElectronID_Fall17_94X_V2_loose",  &gsf_VID_cutBasedElectronID_Fall17_94X_V2_loose);
	outputTree->Branch("gsf_VID_cutBasedElectronID_Fall17_94X_V2_medium",  & gsf_VID_cutBasedElectronID_Fall17_94X_V2_medium);
	outputTree->Branch("gsf_VID_cutBasedElectronID_Fall17_94X_V2_tight", &gsf_VID_cutBasedElectronID_Fall17_94X_V2_tight);
	outputTree->Branch("gsf_VID_cutBasedElectronID_Fall17_94X_V2_veto", &gsf_VID_cutBasedElectronID_Fall17_94X_V2_veto);
	...
	outputTree->Branch("HLT_PFHT450_SixPFJet36_PFBTagDeepCSV_1p59", &HLT_PFHT450_SixPFJet36_PFBTagDeepCSV_1p59, "HLT_PFHT450_SixPFJet36_PFBTagDeepCSV_1p59/O");
...
bool METScanningNtupleMakerMINIAOD::GetIdxFilterDecision(int it){
	...
	else if(it==  idx_PassBadPFMuonFilterDz_Update) return PassBadPFMuonFilterDz_Update;
...
void METScanningNtupleMakerMINIAOD::InitandClearStuff(){
	...
	PassBadPFMuonFilterDz_Update=false;


	HLT_PFHT450_SixPFJet36_PFBTagDeepCSV_1p59 = false;
	HLT_PFHT1050 = false;
	HLT_PFHT900 = false;
	HLT_PFJet500 = false;
	...
	gsf_VID_cutBasedElectronID_Fall17_94X_V2_loose.clear();
	gsf_VID_cutBasedElectronID_Fall17_94X_V2_medium.clear();
	gsf_VID_cutBasedElectronID_Fall17_94X_V2_tight.clear();
	gsf_VID_cutBasedElectronID_Fall17_94X_V2_veto.clear();
